...
const TechInput = ({ variant = 'bordered', onChange }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      } = await axiosInstance.get('/tech-stack');
  ...
  return (
    <Autocomplete label="Select Tech" variant={variant} selectedKey={tagSelect} onSelectionChange={handleValueChange}>
      {tags.map((tag) => (
